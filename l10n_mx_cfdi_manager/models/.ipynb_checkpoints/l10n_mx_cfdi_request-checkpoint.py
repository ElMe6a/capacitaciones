...
class l10n_mx_cfdi_request(models.Model):
    ...
    def create(self,values):
        ...
        # Recibidos
        result = descarga.solicitar_descarga(session, record.rfc_consultant, record.start_date, record.end_date, rfc_receptor=record.rfc_receptor)
    ...
    def _read_fiel(self,keys_id):
        ...
        f = open(file_path,'wb')
        f.write(base64.decodestring(keys_id.fiel))
        f.close()
...
#         raise UserError(key_der)
        ...
        f = open(file_path,'wb')
        f.write(base64.decodestring(keys_id.clave))
        f.close()
...
#     def prueba_conceptos(self):
        
#         documentos = self.env['l10n_mx.cfdi_document'].search([])
        
#         for documento in documentos:
# #             _logger.warning(documento.moves_ids)
#             if not documento.moves_ids:
#                 self.create_bill(documento)
#             desc = None
#             base = 0
#             importe = 0
#             data = self._read_cfdi(documento.attatch)
#             for concepto in data['conceptos']:
#                 if concepto.hasAttribute('Descuento'):
#                     desc = float(concepto.getAttribute('Descuento'))
...
#                 for traslado in traslados:
#                     traslado_float = float(traslado.getAttribute('Base'))
#                     if traslado_float != importe and concepto.getAttribute('Unidad') == 'Litro':
# pass
